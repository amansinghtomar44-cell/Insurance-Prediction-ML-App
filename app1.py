...
# Step 4: Divide datset into features and target
X = df.iloc[:,:-1]
y = df.iloc[:,-1]
...
for index, col_name in enumerate(X.columns):
  ...
  max_v = X[col_name].amx()
  if col_name != "Prevoius_Insurance":
    value = st.sidebar.silder(f"Select value for{col_name}",
                             min_value = min_v,
                             max_value = max_v)
  else:
...
if st.button("Click to Predict: "):
  ...
  if final_ans == 0:
    ...
    st.success("✅Customer will buy Insurance✅")
